...
#             query = text("SELECT rating, genres, original_network, start_year, start_month FROM public.dramas")
...
#         # query = "SELECT start_month, start_year FROM public.dramas WHERE start_year IN (2023, 2024)"
#         # with engine.connect() as conn:
#         #     df = pd.read_sql(text(query), conn)
#         # 2. Gom nhóm: Đếm số phim theo năm và tháng
#         # Vì month đã là số, ta chỉ cần pivot table
#         monthly_counts = df.groupby(['start_year', 'start_month']).size().unstack(level=0, fill_value=0)
        
#         # Đảm bảo đủ 12 tháng (để biểu đồ không bị đứt đoạn nếu có tháng 0 phim)
#         monthly_counts = monthly_counts.reindex(range(1, 13), fill_value=0)

#         # 3. Format dữ liệu cho Recharts (Frontend)
#         # Recharts cần tên tháng để hiển thị trục X đẹp hơn
#         month_names = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"]
#         monthly_releases = []
        
#         for month_num in range(1, 13):
#             monthly_releases.append({
#                 "month": month_names[month_num - 1], # Chuyển 1 -> "Jan"
...
#         # Biểu đồ Xu hướng theo năm (Rating Trends) - Lấy 10 năm gần nhất
...
#         # Biểu đồ Phân bổ điểm (Rating Distribution)
